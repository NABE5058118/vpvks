...
|-------|--------|-------|------|
| **1 месяц** | Безлимитный | 30 дней | 110₽ |
| **3 месяца** | Безлимитный | 90 дней | 290₽ |
| **12 месяцев** | Безлимитный | 365 дней | 500₽ |
...
### 1 марта 2026 — ПЛАТЁЖНАЯ СИСТЕМА И ПРОВЕРКА ПОДПИСКИ

**✅ Добавлено:**

| Функция | Описание |
|---------|----------|
| **Проверка подписки в Mini App** | Ключи выдаются только с активной подпиской |
| **Блокировка без оплаты** | При отсутствии подписки переключение на экран тарифов |
| **Уведомление за 5 дней** | Напоминание об оплате за 5 дней до истечения |
| **Скрипт cron** | `backend/check_expirations.py` для ежедневной проверки |
| **Инструкция YooKassa** | `YOOKASSA_WEBHOOK_SETUP.md` |

**🐛 Исправлено:**

| Проблема | Решение |
|----------|---------|
| Ключи выдавались без проверки оплаты | Добавлена проверка `is_subscription_active()` в Mini App и backend |
...
- `backend/templates/miniapp.html` — проверка подписки
- `backend/services/vpn_service.py` — проверка перед выдачей ключа
- `backend/check_expirations.py` — скрипт уведомлений (новый)
- `backend/tests/test_notifications.py` — тесты
- `bot/main.py` — обновлено приветствие
- `YOOKASSA_WEBHOOK_SETUP.md` — инструкция (новая)
